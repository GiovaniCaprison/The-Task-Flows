...
export const thetaskflowsFaqs: FAQEntry[] = [
  {
    ...
    answer:
      "TheTaskFlows is a powerful automation platform that allows users to efficiently execute SQL-based tasks through a scalable, secure, and easy-to-use interface. By submitting SQL files and custom command scripts, users can leverage pre-configured environments to process their queries without the need for manual setup or infrastructure management. TheTaskFlows eliminates the complexity of configuring environments, enabling users to focus on the execution and analysis of their data workflows. Whether you are running complex queries or batch processing large datasets, TheTaskFlows ensures that tasks are handled quickly and securely, allowing users to achieve results in significantly less time. The platform streamlines SQL task execution, providing a seamless experience that saves users hours of manual effort and configuration, boosting overall productivity and data-driven insights.",
  },
  {
    question: 'What is the expected cost of TheTaskFlows?',
    answer: `As with most cloud-based services, the cost of running TheTaskFlows varies depending on usage patterns. To estimate the cost, we would need to analyze how frequently users submit tasks and the complexity of those tasks. However, TheTaskFlows is designed with cost-efficiency in mind. Running the web app itself typically incurs minimal costs—usually less than $2 per month. The main expenses come from processing larger SQL jobs or tasks that require more computing resources, especially if users are processing significant datasets or running complex queries. While SQL processing itself tends to be relatively inexpensive, users running large-scale operations may incur higher costs, particularly when using persistent database instances or cloud storage. Despite this, TheTaskFlows is optimized to reduce overhead and focus on performance, ensuring users spend more time executing tasks rather than configuring environments, leading to lower overall costs in terms of time and resources.`,
  },
  {
    question: 'Who owns TheTaskFlows?',
    answer:
      'TheTaskFlows is currently a proof of concept (POC) initiated in University whereby Louis Grennell had to create a project for one of his classes. At present, there is no official sponsor, and TheTaskFlows operates without formal support or a dedicated development queue typical of officially funded projects. Despite this, the project is intended to demonstrate the value of automating SQL task execution, with the goal of showcasing its potential as a reliable tool for wider adoption within the organization. The hope is that TheTaskFlows will evolve from its current POC status into a fully supported tool, delivering significant time-saving and automation benefits to data analysts and engineers.',
  },
  {
    question: 'I see this is a POC, what work is still remaining?',
    answer: `Right now, the core architecture of TheTaskFlows is set up and functional. However, before it can become a fully-fledged production ready tool, several areas need further development. Key requirements include ensuring compliance with security standards (e.g., implementing proper AppSec measures) to protect the integrity of SQL processing and command execution. Additionally, we need to improve the scalability of the platform, particularly for load balancing when processing high volumes of SQL tasks, which could be a bottleneck in its current state. There are a few other rough edges, such as optimizing resource allocation and improving the task queueing mechanism, that will need more time to refine before TheTaskFlows can be considered production-ready.`,
  },
  {
...
export const securityFaqs: FAQEntry[] = [
  ...
  {
    question: 'Has the architecture been Anvil certified?',
    answer:
      'Not yet given the project is not fully completed. However, we will obviously be doing a security review in the event TheTaskFlows is formally launched.',
  },
];
